...
exports.handler = async function handler(event) {
  ...
  const folder = (key.startsWith('/') ? key.slice(1) : key)
    .split('/')
    .slice(0, -4)
    .join('/'); // y/m/d/id
  ...
  const dataUrl = await getSignedUrl('getObject', {
    Bucket: bucket,
    Key: key,
    Expires: ONE_HOUR_IN_SECONDS
  });

  const detailsObj = await s3
    .getObject({
      Bucket: bucket,
      Key: key
    })
  ...
  const tagsByFingerprint = fingerprintTagAndDateToUserVisibleTags(
    uid,
    rawTagKeys.map(auditKeyToFingerprintTagAndDate)
  );
